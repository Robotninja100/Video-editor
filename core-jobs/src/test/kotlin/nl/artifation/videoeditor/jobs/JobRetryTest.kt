...
/**
 * De boekhouding rond opnieuw proberen: pogingen, toestand en de bewaarde fout.
 * Hoe lang er tussen twee pogingen gewacht wordt staat in [JobBackoffTest]; hier
 * draait een beleid zonder wachttijd zodat dat de tests niet vertroebelt.
 */
class JobRetryTest {

    private fun queueMet(job: Job): JobQueue =
        JobQueue(openGate(), policy = ZONDER_WACHTTIJD).apply { submit(job) }
    ...
    fun `een herstelbare fout zet de taak terug in de wachtrij`() {
        ...
        val job = queue.fail("export", nowUs = 20L, error = NETWERK_WEG)
    ...
    fun `een blijvende fout is meteen definitief`() {
        ...
        // oordeel komt uit de fout zelf; de wachtrij bedenkt dat niet.
        val job = queue.fail("export", nowUs = 20L, error = BRON_KAPOT)
        ...
        assertNull(job.notBeforeUs, "er valt niets meer te wachten: ${job.notBeforeUs}")
    }

    @Test
    fun `de fout blijft als EditorError bewaard bij het terugleggen`() {
        val queue = queueMet(exportJob("export"))
        queue.startNext(nowUs = 10L)
        val fout = teVaak(retryAfterMs = null)

        val job = queue.fail("export", nowUs = 20L, error = fout)

        assertEquals(fout, job.lastError, "lastError was ${job.lastError}")
        assertEquals("rate_limited", job.lastError?.code, "code was ${job.lastError?.code}")
    }

    @Test
    fun `de bewaarde fout levert een zin op voor de gebruiker`() {
        val queue = queueMet(exportJob("export"))
        queue.startNext(nowUs = 10L)

        val job = queue.fail("export", nowUs = 20L, error = BRON_KAPOT)

        // Dit is de winst van een EditorError boven een vrije string: wat hier
        // staat kan zo op het scherm.
        assertEquals(
            "Het bestand export kan niet gelezen worden. " +
                "Kopieer het opnieuw naar je toestel en probeer het daarna nog eens.",
            job.lastError?.userMessage,
            "userMessage was ${job.lastError?.userMessage}",
        )
    }
    ...
    fun `na het maximum aantal pogingen mislukt de taak definitief`() {
        ...
        repeat(3) { poging ->
            ...
            queue.fail("export", nowUs = 15L + poging, error = NETWERK_WEG)
        }
    ...
    fun `een definitief mislukte taak wordt niet meer gekozen`() {
        val queue = queueMet(exportJob("export"))
        queue.startNext(nowUs = 10L)
        queue.fail("export", nowUs = 20L, error = BRON_KAPOT)

        assertNull(queue.nextCandidate(nowUs = 30L), "een definitief mislukte taak hoort weg te blijven")
    ...
    fun `een geslaagde poging na een fout wist de fout niet uit het logboek`() {
        ...
        assertEquals(NETWERK_WEG, job.lastError, "de eerdere fout hoort zichtbaar te blijven")
