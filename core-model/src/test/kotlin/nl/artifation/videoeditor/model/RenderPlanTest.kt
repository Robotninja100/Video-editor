...
/**
 * Media3 rekent in NDC (−1..1, y omhoog), het model in 0..1 met y omlaag. Die
 * omrekening stond eerder in de Android-module, waar hij alleen met een emulator
 * te toetsen was. Hier kost hij vier tests.
 */
class NdcConversionTest {

    @Test
    fun `het midden van het volledige frame is de oorsprong`() {
        val (x, y) = NormRect.FULL.ndcCenter()
    ...
    fun `de y-as klapt om`() {
        ...
        val (_, y) = NormRect(left = 0f, top = 0f, right = 1f, bottom = 0.5f).ndcCenter()

        assertTrue(y > 0f, "boven in het model hoort positief te zijn in NDC")
        assertEquals(0.5f, y, 1e-5f)
    }

    @Test
    fun `linksboven kwadrant komt goed uit`() {
        val (x, y) = NormRect(0f, 0f, 0.5f, 0.5f).ndcCenter()
    ...
    fun `een gecentreerde 9 op 16 strook blijft gecentreerd`() {
        ...
        assertEquals(0f, x, 1e-5f, "horizontaal gecentreerd")
        assertEquals(0f, y, 1e-5f, "verticaal gecentreerd")
    }
}

/**
 * De masktrack moet synchroon meelopen met het bronmateriaal. Twee dingen konden
 * daarbij misgaan, en gingen ook mis: het in-punt van een getrimde clip, en de
 * snelheid. Die laatste ontbrak volledig, waardoor de blur bij 2× lineair wegliep
 * van het onderwerp — en de spike meet alleen op 1×, dus die zou het niet vangen.
 */
class BronPositieTest {

    private fun blur(
        sourceOffsetUs: Us = 0L,
        sourceEndUs: Us = 60 * SECOND,
        speed: Float = 1f,
    ...
    @Test
    fun `op normale snelheid loopt de bron gelijk op met de cliptijd`() {
        val effect = blur()

        assertEquals(0L, effect.sourcePtsFor(0L))
        assertEquals(5 * SECOND, effect.sourcePtsFor(5 * SECOND))
    }

    @Test
    fun `op dubbele snelheid ligt cliptijd vijf seconden op bron tien`() {
        assertEquals(
            10 * SECOND,
            blur(speed = 2f).sourcePtsFor(5 * SECOND),
    ...
    fun `op halve snelheid loopt de bron half zo snel`() {
        assertEquals(2_500_000L, blur(speed = 0.5f).sourcePtsFor(5 * SECOND))
    }

    @Test
    fun `een getrimde clip telt zijn in-punt erbij`() {
        val effect = blur(sourceOffsetUs = 5 * SECOND)

        assertEquals(
            5 * SECOND,
            effect.sourcePtsFor(0L),
            "op cliptijd nul hoort de mask van het in-punt",
        )
        assertEquals(7 * SECOND, effect.sourcePtsFor(2 * SECOND))
    }

    @Test
    fun `in-punt en snelheid werken samen`() {
        assertEquals(15 * SECOND, blur(sourceOffsetUs = 5 * SECOND, speed = 2f).sourcePtsFor(5 * SECOND))
    }

    @Test
    fun `de bronpositie gaat nooit voorbij het einde van de clip`() {
        val effect = blur(sourceOffsetUs = SECOND, sourceEndUs = 3 * SECOND)

        assertEquals(
            3 * SECOND,
            effect.sourcePtsFor(99 * SECOND),
            "voorbij het einde staat geen mask; de decoder zou doorspoelen",
        )
    }

    @Test
    fun `een negatieve cliptijd levert het in-punt op`() {
        assertEquals(SECOND, blur(sourceOffsetUs = SECOND).sourcePtsFor(-5_000L))
    }

    @Test
    fun `het plan draagt de snelheid van de clip mee naar het blur-effect`() {
        val plan = projectOf(
            clip(
                "a", 4 * SECOND, 8 * SECOND, speed = 2f,
                effects = listOf(EffectSpec.MaskedBlur("file:///m.mp4", radiusFrac = 0.02f)),
        ...
        val effect = assertIs<RenderEffect.MaskedBlur>(plan.firstSource().effects.single())
        assertEquals(2f, effect.speed)
        assertEquals(8 * SECOND, effect.sourceEndUs, "het uit-punt begrenst de mask")
    }
}
