...
internal class MaskVideoDecoder private constructor(
    ...
    companion object {
        ...
        fun createOnGlThread(context: Context, maskUri: String): MaskVideoDecoder {
            ...
            // Vanaf hier worden vijf resources aangemaakt die elkaar niet kennen.
            // `codec.configure` gooit reëel — alle hardwaredecoders bezet, of een
            // profiel dat dit toestel niet kent — en zonder deze catch lekken ze
            // alle vijf, elke renderpas opnieuw.
            var textureId = 0
            ...
            try {
                ...
                callbackThread = HandlerThread("mask-decoder").apply { start() }
                ...
                if (textureId != 0) runCatching { GlUtil.deleteTexture(textureId) }
                runCatching { extractor.release() }
                throw e
            }
        }
