 ...
 * sampler nodig heeft.
 *
 * Het maskframe komt uit [MaskVideoDecoder]. De tijd die daarheen gaat is
 * `presentationTimeUs + sourceOffsetUs`: de renderketen levert **clip**tijd, de
 * maskvideo staat in **bron**tijd. Dat verschil komt kant-en-klaar uit het
 * renderplan mee en wordt hier niet opnieuw uitgerekend — het afleiden uit de
 * clipping-configuratie is precies de fout die pas op een getrimde clip opvalt.
 */
internal class MaskedBlurShaderProgram(
    private val context: Context,
    private val maskUri: String,
    ...
    override fun configure(inputWidth: Int, inputHeight: Int): Size {
        ...
        if (decoder == null) {
            decoder = MaskVideoDecoder.createOnGlThread(context, maskUri)
        }
    ...
    override fun drawFrame(inputTexId: Int, presentationTimeUs: Long) {
        ...
        val hasMask = mask?.advanceTo(presentationTimeUs + sourceOffsetUs) ?: false
...
internal class MaskedBlurEffect(
    ...
    override fun isNoOp(inputWidth: Int, inputHeight: Int): Boolean = radiusFrac <= 0f
}
